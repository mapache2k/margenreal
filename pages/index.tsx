...
export default function ProHome(): JSX.Element | null {
  ...
  // NOTE: guard against missing provider during prerender / server execution
  const pro = usePro();
  if (!pro) {
    // during prerender/use before client hydration the provider may be undefined.
    // Return null so Next's prerender doesn't crash trying to read client state.
    return null;
  }
  ...
  return (
    <ProLayout>
      <style>{`
        ...
        .field label { display: block; font-size: 0.625rem; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: #555; margin-bottom: 7px; line-height: 1; }
        .field input { width: 100%; background: #161614; border: 1.5px solid #2a2a28; color: #f0f0f0; font-family: 'Epilogue', sans-serif; font-size: 0.9375rem; font-weight: 600; padding: 10px 14px; border-radius: 8px; outline: none; transition: border-color 0.18s, box-shadow 0.18s; -moz-appearance: textfield; }
        .field input::-webkit-outer-spin-button, .field input::-webkit-inner-spin-button { -webkit-appearance: none; }
        .field input:focus { border-color: #f0ebe0; box-shadow: 0 0 0 3px rgba(240,235,224,.06); }
        .field input::placeholder { color: #2a2a28; font-weight: 400; }
        .field-row { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        .btn-start { width: 100%; margin-top: 24px; background: #f0ebe0; color: #0a0a0a; font-family: 'Syne', sans-serif; font-weight: 800; font-size: 0.9375rem; padding: 15px; border-radius: 10px; border: none; cursor: pointer; transition: background 0.18s, transform 0.15s, box-shadow 0.2s; letter-spacing: 0.02em; }
        .btn-start:hover:not(:disabled) { background: #fff; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(240,235,224,.15); }
        .btn-start:disabled { opacity: 0.5; cursor: not-allowed; }
        .btn-continue { width: 100%; margin-top: 10px; background: transparent; border: 1px solid #2a2a28; color: #555; font-family: 'Epilogue', sans-serif; font-size: 0.875rem; font-weight: 500; padding: 12px; border-radius: 10px; cursor: pointer; transition: all 0.18s; }
        .btn-continue:hover { border-color: #555; color: #f0f0f0; }
        .hint { font-size: 0.75rem; color: #444; line-height: 1.6; margin-top: 16px; text-align: center; }
      `}</style>
