...
import { upsertPending, type Plan } from '../../../lib/users';
...
<p>Hola,</p>
<p>Tu compra del plan <strong>${planLabel}</strong> fue confirmada. Para activar tu cuenta y configurar tu contraseña, hacé clic acá:</p>
<p style="margin:28px 0;">
  <a href="${link}" style="background:#f9d71b;color:#0a0a0e;font-weight:800;font-size:15px;padding:14px 28px;border-radius:10px;text-decoration:none;display:inline-block;">
    Activar mi cuenta →
  </a>
</p>
<p style="color:#888;font-size:13px;">El enlace es válido por 7 días. Si no hiciste esta compra, ignorá este mensaje.</p>
<p>Saludos,<br>Margen Real</p>
</body></html>`,
      text: `Activá tu cuenta Margen Real ${planLabel}.\n\nEnlace de activación: ${link}\n\nVálido por 7 días.\n\nSaludos,\nMargen Real`,
    }),
  });
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!verifySignature(rawBody, signature)) {
    console.warn('[webhook/lemon] invalid signature');
  ...
  const plan: Plan = PLAN_MAP[variantId] ?? 'starter';

  if (!email) return res.status(400).json({ error: 'No email in payload' });

  upsertPending(email, plan, orderId);
  const token = createActivationToken(email, plan, orderId);
  await sendActivationEmail(email, token, plan);

  console.info(`[webhook/lemon] activation sent to ${email} (${plan})`);
